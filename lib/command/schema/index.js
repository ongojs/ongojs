 ...
 * @requires Cli
 *
 * @classdesc SchemaCommand class
 */

 const Schema = require("@mongodb-model/schema");
class SchemaCommand extends require("@mongodb-model/cli") {
  ...
  constructor(...arrayOfObjects) {
    ...
    // auto bind methods
    this.autobind(SchemaCommand);
    // auto invoke methods
    this.autoinvoker(SchemaCommand);
...
\x1b[36m DESCRIPTION\x1b[0m
 ...
 \x1b[36m  The following options are available: \x1b[0m `;
  ...
  makeSchema() {
    if (this.command(3)) {
      const {makeSchema} = new Schema({comand: this.command(3)})
      if (this.command(4)) {
        return makeSchema(this.command(4));
      } else {
        return makeSchema(this.command(3));
      }
    } else {
      this.man();
    }
