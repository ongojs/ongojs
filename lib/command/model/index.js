...
const HTTPController = require("@ongojs/http-controller");
const TCPController = require("@ongojs/tcp-controller");

const { Green, Red } = new Couleur
class ModelCommand extends require("@ongojs/cli") {
 ...
 \x1b[36m  The following options are available: \x1b[0m `;
      ...
      switch (this.command(5)) {
        case 'h':
          if (this.command(6)) {
            ...
            this.createModelWithHTTPController()
        ...
        case 't':
          if (this.command(6)) {
            ...
            this.createModelWithTCPController()
  ...
  defaultControllerRouteOptions(controllerCommand = 'h') {
    if (this.command(5)) {
      ...
      if(controllerCommand === 't' || controllerCommand === 'tcp'){
        this.createModelWithTCPController()
      }else if(controllerCommand === 'h' || controllerCommand === 'http'){
        this.createModelWithHTTPController()
      }
    }
  }
  ...
  createModelWithHTTPRoute(){}
  createModelWithTCPRoute(){}
  createModelWithHTTPController(){
    this.createModel();
    this.createHTTPController();   
  }
  createModelWithTCPController(){
    this.createModel();
    this.createTCPController();
  }
  createModelWithHTTPControllerAndHTTPRoute(){}
  createModelWithHTCPControllerAndTCPRoute(){}
  createModelWithHTTPControllerAndTCPRoute(){}
  createModelWithTCPControllerAndHTTPRoute(){}
  makeModel() {
    if (this.command(3)) {
      if (this.command(4)) {
        switch (this.command(4)) {
